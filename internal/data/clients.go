...
func ValidateClient(v *validator.Validator, client *Client) {
	...
	v.Check(validator.PermittedValue(client.Status, ClientStatus.ACTIVE, ClientStatus.CONFLICT, ClientStatus.DELETED), "status", fmt.Sprintf("invalid status %s", client.Status))

	v.Check(validator.PermittedValue(client.Shop, Shop.HEADQUARTERS), "shop", fmt.Sprintf("invalid shop %s", client.Shop))
}
...
func (m ClientModel) Insert(client *Client) error {
	...
	args = client.Address.toSQL(args)
...
func (m ClientModel) DeleteExpired() error {
	...
	query := `
		...
		WHERE c.status = $1 AND (c.deleted_at < CURRENT_TIMESTAMP - '3 years'::interval);`
